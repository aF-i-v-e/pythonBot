...
"""Парсинг УБРиР Вклад «Правильный выбор»"""
...
class UbrrBank2:
    ...
    def __init__(self):
        ...
        self._url = "https://www.ubrr.ru/chastnym-klientam/vklady/pravilnyy-vybor"
    ...
    def get_percent(self):
        percents_text = self._response.xpath(
            '//*[@id="accordion-130750"]/div/div[1]/div/table/tbody/tr[1]/td[2]/div/ul/li[1]/strong')[0].text_content()
        return float(percents_text.replace(',', '.')[:-1])

    def get_start_sum(self):
        start_sum_text = self._response.xpath(
            '//*[@id="accordion-130750"]/div/div[1]/div/table/tbody/tr[3]/td[2]/p')[0].text_content()
        start_sum = start_sum_text.split(' ')
        return int(start_sum[0])

    def get_months_number(self):
        months_number_text = self._response.xpath(
            '//*[@id="accordion-130750"]/div/div[1]/div/table/tbody/tr[2]/td[2]/div/ul/li[1]')[0].text_content()
        return int(int(months_number_text.split(' ')[3]) / 30)
